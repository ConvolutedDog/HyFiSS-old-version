...
void iterator(gpointer key, gpointer value, gpointer ekt) {
  HKEY temp;
  // strcpy(temp, key); // yangjianchao16 del
  strcpy((char*)temp, (char*)key); // yangjianchao16 add
...
gboolean compare_strings(gconstpointer a, gconstpointer b) {
  // if (strcmp(a,b) == 0) // yangjianchao16 del
  if (strcmp((const char*)(a), (const char*)(b)) == 0 ) // yangjianchao16 add
    return TRUE;
  else
    return FALSE;
}
