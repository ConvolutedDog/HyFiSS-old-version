...
float parda_fprintf_histogram_r(const unsigned* histogram, FILE* file, bool print);
...
static inline int process_one_access_and_get_distance(char* input, program_data_t* pdt, const long tim) {
    int distance;
    int *lookup;
    lookup = (T*)g_hash_table_lookup(pdt->gh, input);
    //printf("gh=%p process_one\n",pdt->gh);
    ...
    if (lookup == NULL) {
        ...
        *p_data = tim;
        ...
        return B_INF; // return distance = B_INF
    }
    // Hit: We've seen this data before
    else {
        char *data = strdup(input);
        ...
        *p_data = tim;
        ...
        if (distance > nbuckets)
            return B_OVFL; // return distance = B_OVFL
        else
            return distance; // return distance = diatnce
    }
}
#endif
