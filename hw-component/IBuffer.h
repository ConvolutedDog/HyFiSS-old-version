...
class IBuffer {
 public:
  ...
  ibuffer_entry pop_front(unsigned global_all_kernels_warp_id) {
    ...
    m_ibuffer[global_all_kernels_warp_id].erase(m_ibuffer[global_all_kernels_warp_id].begin());
    return entry;
  }
 ...
 private:
  unsigned m_smid;
  /* Every ibuffer entry has two slots, the idx of m_ibuffer is warp_id. */
