...
class Scoreboard {
 public:
  ...
  void reserveRegisters(const unsigned wid, std::vector<unsigned> regnums, bool is_load);
  //当指令完成写回时，其目标寄存器将被释放。
  void releaseRegisters(const unsigned wid, std::vector<unsigned> regnums);
  //将单个目标寄存器释放。
  void releaseRegister(const unsigned wid, const unsigned regnum);
  //检测冒险，检测某个指令使用的寄存器是否被保留在记分板中，如果有的话就是发生了 WAW 或 RAW 冒险。
  bool checkCollision(const unsigned wid, std::vector<unsigned> regnums, 
  ...
  const bool islongop(const unsigned wid, const unsigned regnum);

 private:
  //将单个目标寄存器保留在相应硬件warp的记分牌中。
  void reserveRegister(const unsigned wid, const unsigned regnum);
  //返回SM的ID。
